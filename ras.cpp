...
surface=IMG_Load("res/fireA.png");
  ...
  int playAgain=0;
    ...
    while (isRunning)
    {
        ...
        if (main_game)
        {
             bool stay=true,info=false;
            double initTime=0;
            double startTime = SDL_GetTicks() / 1000.0;
        
            if (startTime  >= 1&&startTime<=10)
            {
                Mix_PlayChannel(-1,rand1,0);
            }
              else if (startTime  >= 13&&startTime<=16)
            {
                if(goob)
                {
                loob=1;
            ...
            else if(startTime>=20&&info==false)
            stay=true;
        ...
        //     } 
            ...
            if(stay==true&&loob==0){
               // int delta=SDL_GetPerformanceCounter();
            SDL_RenderCopy(rend, tex, &playerRect, &playerPosition);
            //SDL_Delay(1000);
         ...
         //  std:: cout <<delta;
             ...
             //SDL_LockTexture(tex,NULL,NULL,NULL);
            SDL_RenderPresent(rend);
